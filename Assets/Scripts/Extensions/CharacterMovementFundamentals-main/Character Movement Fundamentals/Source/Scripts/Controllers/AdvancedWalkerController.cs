...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller
	{
		...
		[SerializeField, Tooltip("Hệ số trượt (nhân với slideGravity). 1 = mặc định.")]
		...
		{
			if (_animator.GetCurrentAnimatorStateInfo(0).IsName("Interact"))
			{
				return;
			}
		...
		void HandleMomentum()
		{
			...
			if (currentControllerState == ControllerState.Grounded)
			{
				_horizontalMomentum = VectorMath.IncrementVectorTowardTargetVector(
					_horizontalMomentum, groundFriction, Time.deltaTime, Vector3.zero);

				float slopeAngle = Vector3.Angle(mover.GetGroundNormal(), tr.up);

				// if (slopeAngle > 0.1f) // chỉ xử lý khi có dốc
				// {
				// 	// 1. Giảm tốc độ di chuyển khi leo dốc (chỉ affect horizontal)
				// 	float slopeFactor = Mathf.Clamp01(1f - (slopeAngle / slopeLimit));
				// 	_horizontalMomentum *= slopeFactor;

				// 	// 2. Thêm slide nhẹ xuống slope
				// 	Vector3 slopeDir = Vector3.ProjectOnPlane(-tr.up, mover.GetGroundNormal()).normalized;
				// 	float slideFactor = slopeAngle / slopeLimit;
				// 	_horizontalMomentum += slopeDir * slideGravity * slideFactor * Time.deltaTime;
				// }
				...
				if (slopeAngle > 0.1f) // chỉ xử lý khi có dốc
				{
					// 1. Giảm tốc độ di chuyển khi leo dốc (chỉ affect horizontal)
					// float slopeFactor = Mathf.Clamp01(1f - (slopeAngle / slopeLimit));
					// slopeFactor = Mathf.Pow(slopeFactor, slopeResistance);
					// // slopeResistance càng cao => leo càng chậm
					// _horizontalMomentum *= slopeFactor;
					// 1. Giảm tốc độ di chuyển khi leo dốc (chỉ affect horizontal)
					float slopeFactor = Mathf.Clamp01(1f - (slopeAngle / slopeLimit));

					// resistance càng cao -> giảm mạnh hơn
					float resistanceFactor = 1f / (1f + slopeResistance * slopeAngle / slopeLimit);

					// kết hợp cả 2
					_horizontalMomentum *= slopeFactor * resistanceFactor;
					movementSpeed *= _horizontalMomentum.magnitude;

					// 2. Thêm slide nhẹ xuống slope
					Vector3 slopeDir = Vector3.ProjectOnPlane(-tr.up, mover.GetGroundNormal()).normalized;
					float slideFactor = slopeAngle / slopeLimit;
					_horizontalMomentum += slopeDir * slideGravity * slideFactor * slideSpeedMultiplier * Time.deltaTime;
				}
				//Debug.Log($"SlopeAngle: {slopeAngle}, HorizontalMomentum: {_horizontalMomentum.magnitude}");
				movementSpeed = storeMovementSpeed;
			}
			else
			{
				_horizontalMomentum = VectorMath.IncrementVectorTowardTargetVector(
					_horizontalMomentum, airFriction, Time.deltaTime, Vector3.zero);
			}
