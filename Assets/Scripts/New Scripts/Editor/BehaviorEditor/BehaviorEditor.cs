    ...
    public class BehaviorEditor : EditorWindow
    {
        ...
        void AddNewNode(Event e)
        {
            ...
            if (settings.currentGraph != null)
            {
				...
				menu.AddItem(new GUIContent("Add Portal"), false, ContextCallback, UserActions.addPortal);
        ...
        void ContextCallback(object o)
        {
            ...
            switch (a)
            {
                case UserActions.addState:
                    ...
                    break;
				case UserActions.addPortal:
                    AddPortalNode(mousePosition);
					break;
                ...
                case UserActions.deleteNode:
                    ...
                    if(selectedNode.drawNode is StateNode)
                    {
                        foreach(BaseNode node in settings.currentGraph.windows)
                        {
                            if(node.enterNode == selectedNode.id)
                            {
                    ...
                    if(selectedNode.drawNode is CommentNode || selectedNode.drawNode is PortalNode)
                    {
                        settings.currentGraph.DeleteNode(selectedNode.id);
                    }
        ...
        public static BaseNode AddStateNode(Vector3 pos)
        {
            BaseNode stateNode = settings.AddNodeOnGraph(settings.stateNode, 200, 100, "State", pos);
            return stateNode;
        }
        ...
        public static BaseNode AddPortalNode(Vector3 pos)
        {
            BaseNode portalNode = settings.AddNodeOnGraph(settings.portalNode, 100, 80, "Portal", pos);
            return portalNode;
        }

        public static BaseNode AddCommentNode(Vector3 pos)
        {
            BaseNode commentNode = settings.AddNodeOnGraph(settings.commentNode, 200, 100, "Comment", pos);
            commentNode.comment = "Fill  the decription";
            return commentNode;
        }
