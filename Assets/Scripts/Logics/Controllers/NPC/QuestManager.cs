...
public class QuestManager : MonoBehaviour
{
    ...
    public void QuestRequest(QuestObject NPCQuestObject)
    {
        ...
        for (int i = 0; i < currentQuestList.Count; i++)
        {
            ...
            {
                if (currentQuestList[i].id == NPCQuestObject.receivableQuestIDs[j] && currentQuestList[i].progress == Quest.QuestProgress.ACCEPTED || currentQuestList[i].progress == Quest.QuestProgress.COMPLETED)
                {
    ...
    //ADD ITEMS
    public void AddQuestItems(string questObject, int itemAmount)
    {
        for (int i = 0; i < currentQuestList.Count; i++)
        {
